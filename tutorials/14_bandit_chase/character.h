...
#include "sprites/key.c"
#include "sprites/bandit_sprite.c"
...
void key_init(Key* k, uint8_t x, uint8_t y){
    ...
    set_sprite_data(3, 1, key_sprite);
    set_sprite_tile(1, 3);
    move_sprite(1, k->x, k->y);
}

void bandit_init(Bandit* b, uint8_t x, uint8_t y){
    b->x = x;
    b->y = y;

    set_sprite_data(4, 1, bandit_sprite);
    set_sprite_tile(2, 4);
    move_sprite(2, b->x, b->y);
}
